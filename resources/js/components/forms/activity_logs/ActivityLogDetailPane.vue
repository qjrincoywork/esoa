 ...
 *
 * A row in the table says that something changed; this says what. The change table is
 * the point of the pane — old value beside new, one row per field — with the request
 * it arrived on underneath, and the rest of the batch last so a single entry can be
 * read as part of the action that produced it.
 */
import { computed, ref, watch } from 'vue';
import { Badge } from '@/components/ui/badge';
import { ArrowRight, Info } from 'lucide-vue-next';
import { useActivityLogs, type ActivityLogDetail, type ActivityLogRow } from '@/composables/activityLogs';
...
const { openActivityLog } = useActivityLogs();

const entry = ref<ActivityLogDetail | null>(props.detail);

// The pane reuses this component when another entry is opened, so setup runs once
// while the props change underneath; mirror them rather than snapshotting.
watch(() => props.detail, (next) => { entry.value = next; });

/** Prefer the fetched entry, fall back to the row so the pane is never empty. */
const shown = computed(() => entry.value ?? (props.row as unknown as ActivityLogDetail));

const changes = computed(() => entry.value?.changes ?? []);
const siblings = computed(() => entry.value?.batch_siblings ?? []);
const context = computed(() => entry.value?.context ?? null);

/**
 * Event colours, kept beside the badge that uses them rather than in the enum: which
...
const eventClass = (event: string | null): string =>
...
const summaryRows = computed(() =>
  [
    ...
    { label: 'Email', value: entry.value?.causer_email },
...
<template>
  <div class="flex w-full flex-col gap-4">
    ...
    <section v-if="siblings.length">
      <h4 class="mb-1.5 text-sm font-medium">
        Also changed in this action
        <span class="font-normal text-[var(--color-text-muted)]">({{ siblings.length }})</span>
      </h4>
      <ul class="flex flex-col gap-1.5">
        <li v-for="sibling in siblings" :key="sibling.id">
          <button
            type="button"
            class="w-full rounded-md border border-[var(--color-border)] px-2 py-1.5 text-left text-sm transition-colors hover:border-[var(--primary-color)]"
            @click="openActivityLog(sibling)">
            <span class="flex items-center gap-1.5">
              <Badge :class="eventClass(sibling.event)" class="shrink-0 border-transparent">
                {{ sibling.event_label }}
              </Badge>
              <span class="truncate">{{ sibling.description }}</span>
            </span>
          </button>
        </li>
      </ul>
